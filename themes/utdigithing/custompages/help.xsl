...
<p><a href="http://www.lib.utexas.edu/services/instruction/learningmodules/utdr/submission_transcript.pdf">Click here for a video transcript</a> (PDF format)</p>
	...
	<p>To browse a specific community or collection, <a href="https://repositories.lib.utexas.edu/community-list">navigate to that community or collection</a> and use the browse menu on the right side of the screen</p>
	...
	<p>To search all of the Repository, use the search box at the top of the navigation bar on the right. When two or more words are entered into the search box, the search automatically retrieves works that contain all the words in the search string. For example, if you enter the two words, cats dogs, into the search box, it will retrieve all works that contain BOTH the words "cats" and "dogs". To search on an exact phrase, use quotations around the phrase. (See below for more search tips.) </p>
